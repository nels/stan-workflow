...
\mu_{i} &= \alpha + \beta_{i} \times X \\
\nu & \sim \textrm{Gamma}(2, 1) \\
\sigma & \sim \textrm{Cauchy}(0, 2)  \\
\alpha & \sim \textrm{Normal}(10, 50) \\
\beta_{i} & \sim \textrm{Normal}(\beta_{\mu,i}, 20) \\
\beta_{\mu,i} & \sim \textrm{Normal}(0, 30) \\
\end{align*}
$$
...
stan_model_code <- "
  ...
  data {                      
    ...
    matrix[N, K] X;     	        // Predictor variable X by IndividualPredictor a.k.a. `predicted`
    vector<lower=0>[N] Y;         // Target variable Y a.k.a.`actual`
